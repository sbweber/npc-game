...
void Sprite::interact()
{
  ...
  if (pos->getTile(facing) && pos->getTile(facing)->getSprite())
    ...
    {
      pos->getTile(facing)->getSprite()->setTile(NULL); //current npc test type: kills self on interaction. Test successful.
      pos->getTile(facing)->setSprite(NULL);
    }//do nothing unless there's a tile in the correct direction with a unit on it (can only ineract with units right now)
...
void Sprite::move(dir d)
{
  ...
  if (pos->getTile(d) && pos->getTile(d)->getIsPassable() && !pos->getTile(d)->getSprite())
  {
    pos->setSprite(NULL);
    pos = pos->getTile(d);
    pos->setSprite(this);
  }//move if possible target tile exists, can be entered, and doesn't have a unit in it
}//void Sprite::move(dir d)
...
void Sprite::setTile(Tile* tile)
{
  pos = tile;
  if (pos)
    pos->setSprite(this); //places unit even if tile is impassible or occupied!
}//void Sprite::setTile(Tile* tile)
